...
def figMetaData(file_path):
    ...
    longitude = {
        "coordinate" : img_doc["gps_longitude"],
        "coordinate_ref" : img_doc["gps_longitude_ref"]
    }

    latitude = {
        "coordinate" : img_doc["gps_latitude"],
        "coordinate_ref" : img_doc["gps_latitude_ref"]
    }
...
if __name__ == "__main__":
    ...
    if any(path.endswith(ext) for ext in (".jpg", ".bmp", ".jpeg",)):
        lat, lon = figMetaData(path)
        webbrowser.open_new_tab(f"http://www.google.com/maps/place/{lat},{lon}")
